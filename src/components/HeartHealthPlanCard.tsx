...
export interface HeartHealthPlanData {
  id?: string;
  title: string;
  description: string;
  ...
  features: string[];
  cta: string;
  variant?: 'heart-health' | 'womens-health' | 'fatty-liver' | 'diabetes-management' | 'weight-management' | 'standard' | 'premium';
}

export const HEART_HEALTH_PLAN_DATA: HeartHealthPlanData = {
  id: 'heart-health',
  title: 'Heart Health Plan',
  description: 'Coaching for cholesterol management and blood pressure control',
...
export function PlanTitle({ title }: { title: string }) {
  return (
    <h3 
      id="heart-health-plan-title"
      className="text-[17px] sm:text-[18.5px] font-bold text-[#C8102E] leading-snug tracking-tight mb-1.5"
    >
      {title}
    </h3>
  );
...
export function PlanDescription({ description }: { description: string }) {
  return (
    <p 
      id="heart-health-plan-description"
...
export function Pricing({ 
  ...
  return (
    <div id="heart-health-plan-pricing-section" className="mb-4 pb-1">
...
export function IncludedHeading({ label = "WHAT'S INCLUDED:" }: { label?: string }) {
  return (
    <div id="heart-health-plan-included-heading" className="mb-3 pt-2 border-t border-neutral-200">
...
export function FeatureItem({ text }: { text: string }) {
  return (
    <li className="flex items-start gap-2.5 text-[#122033]">
      <div 
        className="w-[16px] h-[16px] sm:w-[17px] sm:h-[17px] rounded-full bg-[#C8102E] flex items-center justify-center shrink-0 mt-0.5"
        aria-hidden="true"
      >
...
export function FeatureList({ features }: { features: string[] }) {
  return (
    <ul id="heart-health-plan-features" className="space-y-2.5 sm:space-y-3 mb-6 flex-grow">
...
export function SelectPlanButton({ 
  ...
  return (
    <button
      id="heart-health-plan-select-btn"
      type="button"
      onClick={onClick}
      className="w-full min-h-[44px] py-2.5 px-4 rounded-xl sm:rounded-2xl bg-white hover:bg-[#FFF0F0] active:bg-[#FFE0E0] text-[#122033] border border-neutral-200 hover:border-[#C8102E] text-[12.5px] sm:text-[13px] font-semibold transition-all duration-200 shadow-sm hover:shadow flex items-center justify-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-[#C8102E] focus:ring-offset-2 mt-auto"
    >
      <span>{label}</span>
...
export function HeartHealthPlanCard({
  plan = HEART_HEALTH_PLAN_DATA,
  className = '',
  onSelect
}: {
  plan?: HeartHealthPlanData;
  className?: string;
  onSelect?: (planId: string) => void;
}) {
  const handleSelectClick = () => {
    if (onSelect) {
      onSelect(plan.id || 'heart-health');
    }
  ...
  return (
    <article
      id="heart-health-plan-card"
      aria-label={plan.title}
      className={`relative flex flex-col w-full max-w-[360px] sm:max-w-[380px] mx-auto bg-white border border-neutral-200 hover:border-[#C8102E] rounded-[22px] sm:rounded-[24px] p-5 sm:p-6 shadow-sm hover:shadow-md transition-all text-left ${className}`}
    >
      {/* PlanTitle */}
