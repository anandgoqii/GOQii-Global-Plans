...
export interface GutHealthPlanData {
  id?: string;
  title: string;
  description: string;
  ...
  features: string[];
  cta: string;
  variant?: 'gut-health' | 'heart-health' | 'womens-health' | 'fatty-liver' | 'diabetes-management' | 'weight-management' | 'standard' | 'premium';
}

export const GUT_HEALTH_PLAN_DATA: GutHealthPlanData = {
  id: 'gut-health',
  title: 'Gut Health Plan',
  description: 'Relief from digestive issues including IBS, GERD and IBD',
  originalPrice: '₹23,999',
  savings: 'SAVE 20%',
  price: '₹ 19,199',
  billing: 'Annual',
  monthlyPrice: '₹1,600 / month',
...
export function PlanTitle({ title }: { title: string }) {
  return (
    <h3 
      id="gut-health-plan-title"
      className="text-[17px] sm:text-[18.5px] font-bold text-[#056B47] leading-snug tracking-tight mb-1.5"
    >
      {title}
    </h3>
  );
...
export function PlanDescription({ description }: { description: string }) {
  return (
    <p 
      id="gut-health-plan-description"
...
export function Pricing({ 
  ...
  return (
    <div id="gut-health-plan-pricing-section" className="mb-4 pb-1">
...
export function IncludedHeading({ label = "WHAT'S INCLUDED:" }: { label?: string }) {
  return (
    <div id="gut-health-plan-included-heading" className="mb-3 pt-2 border-t border-neutral-200">
...
export function FeatureItem({ text }: { text: string }) {
  return (
    <li className="flex items-start gap-2.5 text-[#122033]">
      <div 
        className="w-[16px] h-[16px] sm:w-[17px] sm:h-[17px] rounded-full bg-[#056B47] flex items-center justify-center shrink-0 mt-0.5"
        aria-hidden="true"
      >
...
export function FeatureList({ features }: { features: string[] }) {
  return (
    <ul id="gut-health-plan-features" className="space-y-2.5 sm:space-y-3 mb-6 flex-grow">
...
export function SelectPlanButton({ 
  ...
  return (
    <button
      id="gut-health-plan-select-btn"
      type="button"
      onClick={onClick}
      className="w-full min-h-[44px] py-2.5 px-4 rounded-xl sm:rounded-2xl bg-white hover:bg-[#F2FAF5] active:bg-[#E2F5E9] text-[#122033] border border-neutral-200 hover:border-[#056B47] text-[12.5px] sm:text-[13px] font-semibold transition-all duration-200 shadow-sm hover:shadow flex items-center justify-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-[#056B47] focus:ring-offset-2 mt-auto"
    >
      <span>{label}</span>
      <ArrowRight className="w-3.5 h-3.5 text-[#056B47]" />
    </button>
  );
}

export function GutHealthPlanCard({
  plan = GUT_HEALTH_PLAN_DATA,
  className = '',
  onSelect
}: {
  plan?: GutHealthPlanData;
  className?: string;
  onSelect?: (planId: string) => void;
}) {
  const handleSelectClick = () => {
    if (onSelect) {
      onSelect(plan.id || 'gut-health');
    }
  ...
  return (
    <article
      id="gut-health-plan-card"
      aria-label={plan.title}
      className={`relative flex flex-col w-full max-w-[360px] sm:max-w-[380px] mx-auto bg-white border border-neutral-200 hover:border-[#056B47] rounded-[22px] sm:rounded-[24px] p-5 sm:p-6 shadow-sm hover:shadow-md transition-all text-left ${className}`}
    >
      {/* PlanTitle */}
