...
export interface FattyLiverPlanData {
  id?: string;
  title: string;
  description: string;
  ...
  features: string[];
  cta: string;
  variant?: 'fatty-liver' | 'diabetes-management' | 'weight-management' | 'standard' | 'premium';
}

export const FATTY_LIVER_PLAN_DATA: FattyLiverPlanData = {
  id: 'fatty-liver',
  title: 'Fatty Liver Plan',
  description: 'Comprehensive coaching for NAFLD and FLD management',
  originalPrice: '₹25,999',
  savings: 'SAVE 20%',
  price: '₹ 20,799',
  billing: 'Annual',
  monthlyPrice: '₹1,733 / month',
...
export function PlanTitle({ title }: { title: string }) {
  return (
    <h3 
      id="fatty-liver-plan-title"
      className="text-[17px] sm:text-[18.5px] font-bold text-[#B95512] leading-snug tracking-tight mb-1.5"
    >
      {title}
    </h3>
  );
...
export function PlanDescription({ description }: { description: string }) {
  return (
    <p 
      id="fatty-liver-plan-description"
...
export function Pricing({ 
  ...
  return (
    <div id="fatty-liver-plan-pricing-section" className="mb-4 pb-1">
...
export function IncludedHeading({ label = "WHAT'S INCLUDED:" }: { label?: string }) {
  return (
    <div id="fatty-liver-plan-included-heading" className="mb-3 pt-2 border-t border-neutral-200">
...
export function FeatureItem({ text }: { text: string }) {
  return (
    <li className="flex items-start gap-2.5 text-[#122033]">
      <div 
        className="w-[16px] h-[16px] sm:w-[17px] sm:h-[17px] rounded-full bg-[#B95512] flex items-center justify-center shrink-0 mt-0.5"
        aria-hidden="true"
      >
...
export function FeatureList({ features }: { features: string[] }) {
  return (
    <ul id="fatty-liver-plan-features" className="space-y-2.5 sm:space-y-3 mb-6 flex-grow">
...
export function SelectPlanButton({ 
  ...
  return (
    <button
      id="fatty-liver-plan-select-btn"
      type="button"
      onClick={onClick}
      className="w-full min-h-[44px] py-2.5 px-4 rounded-xl sm:rounded-2xl bg-white hover:bg-[#FFFDF7] active:bg-[#FFF5D6] text-[#122033] border border-neutral-200 hover:border-[#B95512] text-[12.5px] sm:text-[13px] font-semibold transition-all duration-200 shadow-sm hover:shadow flex items-center justify-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-[#B95512] focus:ring-offset-2 mt-auto"
    >
      <span>{label}</span>
      <ArrowRight className="w-3.5 h-3.5 text-[#B95512]" />
    </button>
  );
}

export function FattyLiverPlanCard({
  plan = FATTY_LIVER_PLAN_DATA,
  className = '',
  onSelect
}: {
  plan?: FattyLiverPlanData;
  className?: string;
  onSelect?: (planId: string) => void;
}) {
  const handleSelectClick = () => {
    if (onSelect) {
      onSelect(plan.id || 'fatty-liver');
    }
  ...
  return (
    <article
      id="fatty-liver-plan-card"
      aria-label={plan.title}
      className={`relative flex flex-col w-full max-w-[360px] sm:max-w-[380px] mx-auto bg-white border border-neutral-200 hover:border-[#B95512] rounded-[22px] sm:rounded-[24px] p-5 sm:p-6 shadow-sm hover:shadow-md transition-all text-left ${className}`}
    >
      {/* PlanTitle */}
